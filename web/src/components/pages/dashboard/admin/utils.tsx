...
const quickStats = [
    { label: "Total Users", value: "1,294", trend: "+8% vs last week", bgColor: "bg-primary/10", border: "border-primary/10" },
    { label: "Active Sessions", value: "412", trend: "–3% vs last week", bgColor: "bg-blue-100/10", border: "border-blue-100/10" },
    { label: "Pending Invites", value: "27", trend: "+5 new today", bgColor: "bg-amber-100/10", border: "border-amber-100/10" },
];
...
const mockBanSummary: BanSummary = {
    ...
    events: [
        { id: "ban-1", name: "Aayush Gupta", action: "ban", note: "Chargeback detected", timeAgo: "2h ago" },
        { id: "ban-2", name: "Neha Rao", action: "restore", note: "Manual review", timeAgo: "4h ago" },
    ],
};
...
const buildRecentUsers = (rows: UserRow[], limit = 5): RecentUserItem[] => {
    ...
    const sorted = [...rows].sort((a, b) => {
        const aDate = new Date(a.sourceUser?.createdAt ?? a.sourceUser?.updatedAt ?? Date.now()).getTime();
        const bDate = new Date(b.sourceUser?.createdAt ?? b.sourceUser?.updatedAt ?? Date.now()).getTime();
        return bDate - aDate;
    });
    return sorted.slice(0, limit).map((row) => ({
        ...
        role: row.roleLabel,
        status: row.status,
        timeAgo: formatRelativeTime(row.sourceUser?.createdAt ?? row.sourceUser?.updatedAt),
    }));
};
...
const buildActivityFeed = (rows: UserRow[]): ActivityFeedItem[] => {
    ...
    const timeline: ActivityFeedItem[] = [...rows]
        .sort((a, b) => {
            const aDate = new Date(a.sourceUser?.updatedAt ?? a.sourceUser?.createdAt ?? Date.now()).getTime();
            const bDate = new Date(b.sourceUser?.updatedAt ?? b.sourceUser?.createdAt ?? Date.now()).getTime();
            return bDate - aDate;
        })
        .slice(0, 3)
        .map((row, index): ActivityFeedItem => ({
            id: `timeline-${row.id}-${index}`,
            title: row.status.label,
            description: `${row.name} • ${row.roleLabel}`,
            timestamp: formatRelativeTime(row.sourceUser?.updatedAt ?? row.sourceUser?.createdAt),
    ...
    const activeCount = rows.filter((row) => row.status.label === "Active").length;
    const pendingCount = rows.filter((row) => row.status.label.toLowerCase().includes("pending")).length;
    const bannedCount = rows.filter((row) => row.status.label.toLowerCase().includes("ban")).length;
    ...
    if (activeCount) {
        aggregate.push({
            ...
            description: `${activeCount} verified`
...
const buildCourseInsights = (rows: UserRow[]): CourseInsight[] => {
    if (!rows?.length) return mockCourseInsights;
    const instructorCount = rows.filter((row) => row.roleLabel.toLowerCase().includes("instructor")).length;
    const managerCount = rows.filter((row) => row.roleLabel.toLowerCase().includes("manager")).length;
    ...
    return [
        {
            id: "courses-live",
            title: "Live cohorts",
            value: `${Math.max(5, instructorCount * 2 + 4)}`,
            subtext: `${Math.max(1, instructorCount)} instructor squads live`,
            progress: 65,
            accent: "text-primary",
            trend: `+${Math.max(1, managerCount)} vs last week`,
        },
        {
            id: "courses-building",
            title: "In production",
            value: `${Math.max(3, Math.round(total / 4))}`,
            subtext: "Content in QA & review",
            progress: 48,
            accent: "text-amber-600",
            trend: "launch window: 2 weeks",
        },
        {
            id: "courses-complete",
            title: "Graduated learners",
...
const buildBanSummary = (rows: UserRow[]): BanSummary => {
    if (!rows?.length) return mockBanSummary;
    const bannedRows = rows.filter((row) => row.status.label.toLowerCase().includes("ban"));
    const restoredRows = rows.filter((row) => !row.status.label.toLowerCase().includes("ban"));

    const bannedEvents = bannedRows.slice(0, 2).map((row, index) => ({
        id: `ban-${row.id}-${index}`,
        name: row.name,
        action: "ban" as const,
        note: row.roleLabel,
        timeAgo: formatRelativeTime(row.sourceUser?.updatedAt ?? row.sourceUser?.createdAt),
    }));

    const restoredEvents = restoredRows.slice(0, 2).map((row, index) => ({
        id: `restore-${row.id}-${index}`,
        name: row.name,
    ...
    const events = [...bannedEvents, ...restoredEvents];

    return {
        totalBanned: bannedRows.length,
        totalRestored: restoredRows.length,
        deltaLabel: `${events.length} actions tracked`,
        events: events.length ? events : mockBanSummary.events,
    };
};
